...
Seven Lambda functions, each with its own IAM role, dependencies, and build step. The Strands
agent itself is **not** here — it's a container on AgentCore Runtime, see `../agent/`.

| Folder | Function name | Trigger | Purpose |
| --- | --- | --- | --- |
| `api/` | `security-triage-api` | API Gateway | Validates Cognito JWT, proxies chat to the AgentCore Runtime, handles task queue CRUD and compliance workspace routes |
| `agent-tools/` | `security-triage-agent-tools` | Invoked by the Strands agent (`../agent/`) | Executes all agent tools — reads Security Hub, GuardDuty, CloudTrail, Config, IAM, Cost Explorer, Access Analyzer |
...
| `compliance-repair/` | `security-triage-compliance-repair` | EventBridge (every 5 min) + SQS DLQ | Marks stuck IN_PROGRESS jobs FAILED after 16 min; redrives DLQ messages for transient failures |
...
cd lambda/compliance-repair && npm run build
```

`deploy.sh` runs all builds automatically. The agent container (`../agent/`) is built separately
by CDK (`DockerImageAsset`) during `cdk deploy` — see `../agent/` and `cdk/README.md`.

## Architecture rules

- `api/` — may call DynamoDB and the AgentCore Runtime. Never calls AWS service APIs directly.
...
- `compliance-repair/` — only writes DynamoDB status fields. No Bedrock, no S3, no Security Hub access.
